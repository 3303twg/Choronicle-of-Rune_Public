...
public class StatModifier : MonoBehaviour  //패시브로 인한 스텟 변동치
{
    ...
    private bool onOffEbonyMagic; private float ebonyMagicBonus; private int ebonyMagicCount;
    private bool onOffHeavyWallet; private float heavyWalletBonus; private int heavyWalletStack;
    ...
    public void ResetStatModifier(object param)
    {
        ...
        onOffEbonyMagic = false; ebonyMagicBonus = 0; ebonyMagicCount = 0;
        EventBus.Unsubscribe("MonsterDead", EbonyMagicStack);
        
        onOffHeavyWallet =  false; heavyWalletBonus = 0; heavyWalletStack = 0;
        EventBus.Unsubscribe("GetItem", GetItem);
    }
    ...
    {
        get{  return attackPower + (onOffMoneyShot ? moneyShotBonusAtk : 0) + (onOffEbonyMagic ? ebonyMagicCount * ebonyMagicBonus : 0); }
        ...
        else
        {
            ...
            foreach (var kvp in slotSkills)
            {
                foreach (RuneSlotController runeSlot in kvp.runeSlots)
                {
                    ...
                    {
                        ElementalType type = elementalRune.GetElemental();
                        Debug.Log($"룬 속성: {type}");
                        typeSet.Add(type); // ✅ 이 부분이 핵심
                    }
                    else
                    {
                        Debug.Log("속성 없음");
            ...
            if (onOffElementalOverCharge)
                elementalOverChargeCheck = typeSet.Count == 1;
        }
